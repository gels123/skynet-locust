...
function playerDataFileWriter:ctor()
    ...
    -- 文件名
    self.fileName = string.format("%s%s", "mysql_error", playerDataCenter.svrIdx)
    -- 最大文件大小
...
function playerDataFileWriter:init(fileDir, fileName)
    Log.i("==playerDataFileWriter:init begin==", playerDataCenter.svrIdx)
    -- 文件路径
    if fileDir then
    ...
    -- 文件名
    if fileName then
        self.fileName = string.format("%s%s", fileName, playerDataCenter.svrIdx)
    end
    Log.i("==playerDataFileWriter:init end==", playerDataCenter.svrIdx)
end
...
function playerDataFileWriter:newFiles()
    ...
    Log.i("playerDataFileWriter:newFiles fileDir=", self.fileDir, "fileName=", self.fileName)
    local infoFilename = self:getNewFile(self.fileDir, self.fileName)
    Log.i("playerDataFileWriter:newFiles infoFilename=", infoFilename)
    -- 打开文件
    self.infoFile = io.open(infoFilename, "w")
    if not self.infoFile then
        Log.e("playerDataFileWriter:newFiles error: open file failed.", infoFilename)
    else
        -- 设置缓冲大小
        self.infoLen = 0
        self.infoFile:setvbuf("full", 8192)
        Log.i("playerDataFileWriter:newFiles open file success.", infoFilename)
    end
end
...
function playerDataFileWriter:writeFile(str)
    if dbconf.DEBUG then
        Log.d("playerDataFileWriter:writeFile str=", str)
    end
...
function playerDataFileWriter:loadFile1()
    Log.i("playerDataFileWriter:loadFile1 begin", playerDataCenter.svrIdx)
    -- 校验mysql是否异常
    local ret = playerDataCenter:executeSql("select 1")
    -- Log.dump(ret, "playerDataFileWriter:loadFile1 ret=", 10)
    if not ret or ret.err then
        Log.e("playerDataFileWriter:loadFile1 error: mysql exception!")
        return
    end
    -- 遍历读取文件, 并执行文件sql
    local fileArray = {}
    ...
    Log.i("playerDataFileWriter:loadFile1 fileDir=", self.fileDir, "fileName=", self.fileName)
    ...
    table.sort(fileArray, function (a, b) -- 文件按时间由早到晚排序
        ...
        -- Log.d("playerDataFileWriter:loadFile2 sort a=", y1, m1, d1, i1, "b=", y2, m2, d2, i2)
    ...
    Log.i("playerDataFileWriter:loadFile1 #fileArray", #fileArray, transformTableToString(fileArray))
    local successArray = {}
    for _,file in ipairs(fileArray) do
        local filename = string.format("%s/%s", self.fileDir, file)
        for line in io.lines(filename) do
            local tmp = svrFunc.split(line, "#")
            Log.i("playerDataFileWriter:loadFile1 do=", file, tmp[1], tmp[2])
    ...
    for _,file in pairs(successArray) do
        xpcall(function ()
            ...
            Log.i("playerDataFileWriter:loadFile1 shell=", shell)
            local ret = io.popen(shell)
            Log.i("playerDataFileWriter:loadFile1 shell ret=", ret)
...
function playerDataFileWriter:loadFile2()
    Log.i("playerDataFileWriter:loadFile2 begin", playerDataCenter.svrIdx)
    local redisLib = require("redisLib")
    -- 校验redis是否异常
    local pcallOk, ok = function ()
        return redisLib:ping()
    end
    Log.d("playerDataFileWriter:loadFile2 pcallOk=", pcallOk, "ok=", ok)
    if not pcallOk or ok then
        Log.e("playerDataFileWriter:loadFile2 error: redis exception!")
        return
    end
    -- 遍历读取文件, 并执行文件sql
    local fileArray = {}
    ...
    Log.i("playerDataFileWriter:loadFile2 fileDir=", self.fileDir, "fileName=", self.fileName)
    for file in lfs.dir(self.fileDir) do
        if string.find(file, self.fileName) then
            table.insert(fileArray, file)
        end
    ...
    Log.i("playerDataFileWriter:loadFile2 #fileArray", #fileArray, transformTableToString(fileArray))
    local successArray = {}
    for _,file in ipairs(fileArray) do
        xpcall(function ()
            local filename = string.format("%s/%s", self.fileDir, file)
            for line in io.lines(filename) do
                local tmp = svrFunc.split(line, "#")
                Log.i("playerDataFileWriter:loadFile2 do=", file, tmp[1], tmp[2])
                if tmp[1] and tmp[2] then
                    local f = load(tmp[2])
                    if f then
                        skynet.fork(f)
    ...
    for _,file in pairs(successArray) do
        xpcall(function ()
            local filename = string.format("%s/%s", self.fileDir, file)
            local shell = string.format("rm %s", filename)
            Log.i("playerDataFileWriter:loadFile2 shell=", shell)
            local ret = io.popen(shell)
            Log.i("playerDataFileWriter:loadFile2 shell ret=", ret)
        end, svrFunc.exception)
    end
end
