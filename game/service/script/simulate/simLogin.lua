...
function simLogin:connectLogin(host, port, token)
	Log.d("simLogin:connectLogin", host, port, token.user)
	self.token = token
	self.state = S2C_CHALLENGE
...
function simLogin:S2C_CHALLENGE(msg)
	...
	Log.d("simLogin:S2C_CHALLENGE ok, state=", self.state, "clientkey=", self.clientkey)
...
function simLogin:S2C_SERVERKEY(msg)
	...
	Log.d("simLogin:S2C_SERVERKEY ok, state=", self.state, "hmac=", hmac, "b64encodehmac=", b64encodehmac)
...
function simLogin:S2C_HANDSHAKE(msg)
	...
	if code == simLogin.Err_Login_HandshakeFailed then
		...
		Log.d("simLogin:S2C_HANDSHAKE ok, state=", self.state, "sendmsg=", sendmsg)
...
function simLogin:S2C_AUTH_TOKEN(msg)
	...
	if code == simLogin.Err_Login_AuthTokenSuccess then
		...
		local b64kid,b64gatenodeid,b64gateip,b64gateport,b64subid,b64uid,b64isInit = resultmsg:match("([^@]+)@([^@]+)@([^@]+)@([^@]+)@([^@]+)@([^@]+)@(.+)")

		--记录数据
		self.kid = crypt.base64decode(b64kid)
		self.nodeid = crypt.base64decode(b64gatenodeid)
		self.ip = crypt.base64decode(b64gateip)
		self.port = crypt.base64decode(b64gateport)
		self.subid = crypt.base64decode(b64subid)
		self.uid = crypt.base64decode(b64uid)
		self.isInit = crypt.base64decode(b64isInit)
		Log.d("simLogin:S2C_AUTH_TOKEN ok, kid=", self.kid, "nodeid=", self.nodeid, "ip=", self.ip, "port=", self.port, "uid=", self.uid, "subid=", self.subid, "isInit=", self.isInit)

		--通知token认证ok, 连接登陆服务器成功, 下面要开始连接网关
		self:connectLoginSuccess()
	else
		--登陆失败
		Log.d("simLogin:S2C_AUTH_TOKEN fail, code=", code, submsg)
		self:onFailure(code)
	end
end
...
function simLogin:connectLoginSuccess()
	--Log.d("simLogin:connectLoginSuccess", self.token.user)
	self:close()
...
function simLogin:onFailure(code)
	Log.w("simLogin:onFailure user=", self.token.user, "code=", code)
	self:close()
	skynet.exit()
end
