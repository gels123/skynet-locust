...
function simSocket:connect(host, port)
	...
	if fd then
		-- 连接成功
		--print("simSocket:connect success", self.name, self.host, self.port)
		self.fd = fd
		self.connected = true
		self:onConnected()
	else
...
-- @override 连接失败
function simSocket:onFailure(code)
	print("simSocket:onFailure", self.name, self.host, self.port, code)
end
