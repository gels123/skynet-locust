...
function runner.agent_count()
    return agent_count
end

function runner.wsport()
    return wsport
end
...
function runner.run_agent(ver, id_start, id_count, per_sec, host, script, cb)
    print("runner.run_agent ver=", ver, "id_start=", id_start, "id_count=", id_count, "per_sec=", per_sec, "host=", host, "script=", script, "cb=", cb)
...
function runner.start(port, _wsport)
    ...
    socket.start(id , function(sock, addr) 
        ...
        function handle.connect(sock)
            ws_socks[sock] = sock
        end
        function handle.close(sock, code, reason)
            ws_socks[sock] = nil
        end
        function handle.error(sock)
            ws_socks[sock] = nil
        end
