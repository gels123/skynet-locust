...
function redisOpt.sendSet(key, value)
    assertParameters(db, key, value)
    return db:set(key, value)
end
...
function redisOpt.decrby(key, decrement)
    assertParameters(db, key, decrement)
...
function redisOpt.incrby(key, increment)
    assertParameters(db, key, increment)
...
function redisOpt.setex(key, time, value)
     assertParameters(db, key, time, value)
...
function redisOpt.sendDelete(key)
    assertParameters(db, key)
    return db:del(key)
end
...
function redisOpt.sendHSet(h, key, value)
    assertParameters(db, key, value)
    return db:hset(h, key, value)
end
...
function redisOpt.sendHDel(h, key)
    assertParameters(db, key)
    return db:hDel(h, key)
end
