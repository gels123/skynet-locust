...
function timerMgr:ctor(func, timer)
    ...
    if func and type(func) == "function" then
		...
		self.func = function(data)
			Log.e("timerMgr callback func ignore", transformTableToString(data))
		end
    end
	if timer then
		self.timer = timer
		...
		-- 计时器
		self.timer = require("myScheduler").new("timerMgr")
		-- 启动计时器
		self:startTimer()
	end
end
...
function timerMgr:updateTimer(id, timerType, endTime)
	if not id or not timerType then
		Log.e("timerMgr:updateTimer error", id, timerType, endTime)
		return
	end
	local timerId = self.timerMap[id] and self.timerMap[id][timerType]
	if not endTime or endTime <= 0 then
		if timerId then
			Log.i("timerMgr:updateTimer remove 1=", id, timerType, endTime, "timerId=", timerId)
			-- 删除现有计时器
		...
		if timerType ~= "heartbeat" then
			Log.i("timerMgr:updateTimer update 2=", id, timerType, endTime, "timerId=", timerId)
		end
		if timerId then
			-- 更新计时器回调时间
			if not self.timer:reset(timerId, endTime) then
				Log.e("timerMgr:updateTimer error", id, timerType, endTime, "timerId=", timerId)
			end
		else
			-- 新增计时器
...
function timerMgr:removeTimer(id, timerType)
	if not id then
		Log.e("timerMgr:updateTimer error", id, timerType)
		return
	end
	if timerType then
		local timerId = self.timerMap[id] and self.timerMap[id][timerType]
