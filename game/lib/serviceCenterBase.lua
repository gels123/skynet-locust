...
function serviceCenterBase:runTimer()
    ...
    timerUpdate = function()
        xpcall(function()
            if not instance then
                Log.e("serviceCenterBase:runTimer error: no instance", self.kid, self.class.__cname)
            else
                instance.myTimer:update()
            end            
        end, svrFunc.exception)

        skynet.timeout(100, timerUpdate)
    end
...
-- 分发客户端指令
function serviceCenterBase:dispatchmsg(req)
    msgCtrl.handle(req)
end
...
function serviceCenterBase:dispatchCmd(session, source, cmd, ...)
    ...
    if func then
        ...
        self:ret()
        Log.e("serviceCenterBase:dispatchCmd error: cmmand not found:", cmd, ...)
    end
end
