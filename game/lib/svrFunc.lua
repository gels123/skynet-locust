...
function svrFunc.isRandomSuccess(num, min, max)
    ...
    Log.d("randNum=", randNum, randomSeed)
    return randNum <= num
end
...
function svrFunc.binarySearch(t, element)
    ...
    if "table" ~= type(t) then
        Log.i("bad argument #1 to 'svrFunc.find' (array expected, got " .. type(t) .. ")")
        return
    end

    if #t == 0 then
        Log.i("bad argument #1 to 'svrFunc.find' (got a empty array)")
        return
    end

    if not element then
        Log.i("bad argument #2 to 'svrFunc.find' (got a nil")
        return
    end
...
function svrFunc.httpPost(host, data, url)
    --Log.i("svrFunc.httpPost(host, url, data)", host, url, data)
    ...
    --Log.dump(status, "http status=", 10)
    --Log.dump(body, "http body=", 10)
    ...
    if "string" == type(two) and string.len(two) > 0 then
        ...
        for i=1, len do
            ...
            if 0 == tonumber(num) or 1 == tonumber(num) then
                ...
                Log.i("svrFunc.convertToTen: not 0 or 1")
                return nil
            end
        end
    ...
    if "string" == type(two) and string.len(two) > 0 then
        ...
        for i=1, len do
            ...
            elseif 1 == tonumber(num) then
                ...
                Log.i("svrFunc.convertToInverse: not 0 or 1")
                return nil
            end
        end
        return result
...
function svrFunc.fileExists(path)
    ...
    Log.i("svrFunc.fileExists(path)",path, file)
    return file ~= nil
end
    ...
    for k, v in pairs(src) do
        if type(v) == "table" then
            ...
            elseif type(dest[k]) == "table" then
                ...
                Log.i("svrFunc.deepMerge type not same")
            end
        else
            dest[k] = v
        end
...
function svrFunc.memoryRecovery()
    --Log.dump(collectgarbage("count"), "collectgarbage start", 10)
    skynet.send(skynet.self(),"debug", "GC")
    --Log.dump(collectgarbage("count"), "collectgarbage end", 10)
end
...
-- 通知钉钉服务器
function svrFunc.notifyJJIM(text, url)
    if not dbconf.isNotifyJJIM then
        return
    end
    local curlLib = require("luacurl")
    local curl = curlLib.easy()
    local form = string.format("{\"msgtype\":\"text\",\"text\":{\"content\":\"%s\"}}", text)
    local myBuffer = ""
    local mywritedata = {}
    curl:setopt(curlLib.OPT_URL, url)
    curl:setopt(curlLib.OPT_NOSIGNAL, true)
    curl:setopt(curlLib.OPT_CONNECTTIMEOUT, 3)
    curl:setopt(curlLib.OPT_TIMEOUT, 3)
    curl:setopt(curlLib.OPT_POST, true)
    curl:setopt(curlLib.OPT_HTTPHEADER, "Content-Type:application/json;charset=UTF-8")
    curl:setopt(curlLib.OPT_SSL_VERIFYPEER, false) --https
    curl:setopt(curlLib.OPT_SSL_VERIFYHOST, 0) --https
    curl:setopt(curlLib.OPT_POSTFIELDS, form)

    local setretres = curl:setopt(curlLib.OPT_WRITEFUNCTION, function (userparam, buffer)
        myBuffer = myBuffer .. buffer
        return string.len(buffer)
    end)
    curl:setopt(curlLib.OPT_WRITEDATA, mywritedata)
    local ok, curlres, errmsg, errcode = xpcall(handler(curl, curl.perform), svrFunc.exception)
    Log.i("notifyJJIM =", ok, curlres, errmsg, errcode, myBuffer)
    if not ok then
        Log.w("svrFunc.notifyJJIM failed", ok)
        curl:close()
    end
end
