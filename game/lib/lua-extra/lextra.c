...
static char cmd_str[128] = "stopSignal";
...
static void my_signal_handler(int sig) {
    ...
    printf("my_signal_handler signal=%d datasz=%d data=%s\n", sig, strlen(data), data);
...
static int modify_singal_handler(lua_State *L) {
    ...
    printf("modify_singal_handler svrName=%s handler=%d\n", svrName, handler);
...
//重置信号处理函数
static void reset_singal_handler(lua_State *L) {
