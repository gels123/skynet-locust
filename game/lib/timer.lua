...
-- 这样可以保证计时的准确性
-- curTime 参数可选
function timer:getRemainTime(curTime)
    curTime = curTime or svrFunc.systemTime()
...
function timer:dispatchListener()
    ...
    if "function" == type(self.listener_) then
        -- 触发回调
        self.listener_(self.count_, self.userData)
    else
        Log.i("timer listener is not the function!")
    end
end
...
function timer:dump(title)
    Log.i("========" .. (title or "timer:dump") .. "=======")
    Log.i("totalTime = ", self.totalTime_)
    Log.i("startTime = ", self.startTime_)
    Log.i("remainTime = ", self:getRemainTime())
    Log.i("listener = ", self.listener_)
    Log.i("count = ", self.count_)
    Log.i("\n")
end
